...
##With the exposed condition the data was slightly right skewed and we used a Gamma distribution and the glmer function. Trying to figure out how to get t statistic
```{r}
EEEexpMF <- glmer(CO2.prod.uL.min ~ Age + Sex + Mass + (chamber|batch), family= "Gamma", data = mf_exposed)
EEEexpMF

glmer <- glmer(CO2.prod.uL.min ~ Sex + Act + Mass + Age +(chamber|batch), family= "Gamma", data = mf_exposed)
glmer

summary(glmer)
glmernosex <- glmer(CO2.prod.uL.min ~ Act + Age + Mass + (chamber|batch), family= "Gamma", data = mf_exposed)
glmernosex
summary(glmernosex)
...
EEEexpMF<- lmer(CO2.prod.uL.min~Age+Sex+Mass+(chamber|batch), data=mf_exposed)
summary(EEEexpMF)
